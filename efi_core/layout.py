...
from efi_core.types import ChunkerSpec, EmbedderSpec, Finding
...
class LibraryLayout(BaseLayout):
    ...
    Structure:
    - library_root/
      - index.json          # URL to document ID mapping (new structure)
      - findings.json       # All findings data (old structure)
      - metadata.json       # Library metadata
      - documents/          # Document directories (new structure)
        - doc_id/
          - findings.json   # Findings for this document
    ...
    @property
    def index_path(self) -> Path:
        """Get path to index.json file (new structure)"""
        return self.library_root / "index.json"
    
    @property
    def findings_path(self) -> Path:
        """Get path to findings.json file (old structure)"""
    ...
    def documents_dir(self) -> Path:
        """Get path to documents directory (new structure)"""
        return self.library_root / "documents"
    ...
    def doc_findings_path(self, doc_id: str) -> Path:
        """Get path to document findings file"""
        return self.doc_dir(doc_id) / "findings.json"
    
    def doc_metadata_path(self, doc_id: str) -> Path:
        """Get path to document metadata file"""
        return self.doc_dir(doc_id) / "metadata.json"
    
    def get_document_ids(self) -> List[str]:
        """Get list of all document IDs"""
        if not self.documents_dir.exists():
            return []
        return [d.name for d in self.documents_dir.iterdir() if d.is_dir()]
    
    def get_url_mapping(self) -> Dict[str, str]:
        """Get URL to document ID mapping from index"""
    ...
    def get_doc_id_from_finding_id(self, finding_id: str) -> Optional[str]:
        """Get document ID from finding ID using direct extraction"""
        # Extract the document ID part from the finding ID
        doc_id = Finding.extract_doc_id_from_finding_id(finding_id)
        
        # Check if this document ID exists
        if doc_id in self.get_document_ids():
            return doc_id
        
        return None
    
    def generate_doc_id(self, url: str) -> str:
        """Generate a document ID for a given URL"""
        return Finding.generate_doc_id(url)
    
    def get_url_from_doc_id(self, doc_id: str) -> Optional[str]:
        """Get URL from document ID using the index"""
        url_mapping = self.get_url_mapping()
        # Reverse lookup: find URL that maps to this doc_id
        for url, mapped_doc_id in url_mapping.items():
...
class EmbeddedCorpusLayout(CorpusLayout, WorkspaceLayout):
    ...
    def chunks_dir(self, chunker: ChunkerSpec) -> Path:
        """Get base directory for chunks"""
        corpus_name = self.corpus_root.name
        workspace_dir = self.get_corpus_workspace_dir(corpus_name)
        return workspace_dir / "chunks" / chunker.key()
    ...
    def embeddings_dir(self, chunker: ChunkerSpec, embedder: EmbedderSpec) -> Path:
        """Get base directory for embeddings"""
        corpus_name = self.corpus_root.name
        workspace_dir = self.get_corpus_workspace_dir(corpus_name)
        return workspace_dir / "embeddings" / chunker.key() / embedder.key()
    ...
    def doc_state_path(self, doc_id: str) -> Path:
        ...
        corpus_name = self.corpus_root.name
        workspace_dir = self.get_corpus_workspace_dir(corpus_name)
        return workspace_dir / "doc_meta" / f"{doc_id}.meta.json"
...
class EmbeddedLibraryLayout(LibraryLayout, WorkspaceLayout):
    ...
    def chunks_path(self, finding_id: str, chunker: ChunkerSpec) -> Path:
        ...
        library_workspace_dir = self.get_library_workspace_dir(library_name)
        return library_workspace_dir / "chunks" / chunker.key() / f"{finding_id}.chunks.jsonl"
    
    def chunks_dir(self, chunker: ChunkerSpec) -> Path:
        """Get base directory for chunks"""
        library_name = self.library_root.name
        library_workspace_dir = self.get_library_workspace_dir(library_name)
        return library_workspace_dir / "chunks" / chunker.key()
    ...
    def emb_path(self, finding_id: str, chunker: ChunkerSpec, embedder: EmbedderSpec) -> Path:
        ...
        library_name = self.library_root.name
        library_workspace_dir = self.get_library_workspace_dir(library_name)
        return library_workspace_dir / "embeddings" / chunker.key() / embedder.key() / f"{finding_id}.npy"
    
    def embeddings_dir(self, chunker: ChunkerSpec, embedder: EmbedderSpec) -> Path:
        """Get base directory for embeddings"""
        library_name = self.library_root.name
        library_workspace_dir = self.get_library_workspace_dir(library_name)
        return library_workspace_dir / "embeddings" / chunker.key() / embedder.key()
