    ...
    try:
        ...
        for i, (premise, hypothesis) in enumerate(test_cases):
            ...
            if i >= 2:  # Long text and meaningful content cases
                assert total_score > 0, f"NLI scorer returned all zeros for edge case {i}: premise='{premise[:50]}...', hypothesis='{hypothesis[:50]}...'"

                # Ensure only one category has non-zero score for meaningful cases
                non_zero_count = sum(1 for score in scores.values() if score > 0)
                assert non_zero_count == 1, f"NLI scorer expected 1 non-zero score, got {non_zero_count} for edge case {i}"
