...
from efi_core.types import LibraryDocument, LibraryDocumentWFindings, Finding
from efi_core.protocols import LibraryBuilder as LibraryBuilderProtocol

logger = logging.getLogger(__name__)


class LibraryBuilder(LibraryBuilderProtocol):
    ...
    def __init__(self, 
        ...
        Args:
            ...
            store: Store to use for saving findings
        ...
        logger.info(f"  Store: {store.__class__.__name__}")
    ...
    def build_library(self, 
        ...
        logger.info(f"  Total findings: {summary['total_findings_count']}")
        
        return summary
    
    def _process_urls(self, urls: List[str]) -> List[LibraryDocumentWFindings]:
        ...
        for url in urls:
            try:
                ...
                for extractor in self.extractors:
                    try:
                        ...
                        if findings and findings.findings:
                            # Generate finding IDs for each finding
                            for i, finding in enumerate(findings.findings):
                                finding.finding_id = Finding.generate_id(url, i)
                            
                            successful_extractor = extractor.name
                            findings.metadata['extractor_used'] = extractor.name
                ...
                if findings and findings.findings:
                    storage_success = self.store.store_findings(findings)
                    if storage_success:
                    ...
                    failed_findings = LibraryDocumentWFindings(
                        doc_id=doc_id,
                        url=url,
                        findings=[],
                ...
                doc_id = Finding.generate_doc_id(url)
                error_findings = LibraryDocumentWFindings(
                    doc_id=doc_id,
                    url=url,
                    findings=[],
    ...
    def _get_build_summary(self, results: List[LibraryDocumentWFindings]) -> Dict[str, Any]:
        ...
        return {
            ...
            'store_info': self.store.get_storage_stats()
        }
    ...
    def get_library_stats(self) -> Dict[str, Any]:
        ...
        store_stats = self.store.get_storage_stats()
    ...
    def export_library(self, export_path: Optional[Path] = None) -> Path:
        ...
        # Get all findings
        all_findings = self.store.list_all_findings()
